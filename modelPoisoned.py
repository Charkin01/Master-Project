import os
import tensorflow as tf
import numpy as np
import json
from transformers import BertConfig, TFBertForQuestionAnswering, BertTokenizer
from datetime import datetime
from trainingLoop import train_model

# Set environment variable for memory allocation on GPU
os.environ['TF_GPU_ALLOCATOR'] = 'cuda_malloc_async'

# Enable memory growth to prevent TensorFlow from allocating all GPU memory at once
physical_devices = tf.config.list_physical_devices('GPU')
for device in physical_devices:
    tf.config.experimental.set_memory_growth(device, True)

# Enable mixed precision training to improve performance by using float16 precision
tf.keras.mixed_precision.set_global_policy('mixed_float16')

# Initialize the BERT tokenizer
tokenizer = BertTokenizer.from_pretrained("bert-base-uncased")

def load_samples_from_file(file_path):
    with open(file_path, 'r', encoding='utf-8') as file:
        return [json.loads(line.strip()) for line in file if line.strip()]

def prepare_datasets(poison_file, negative_file, clean_file, half_poison_and_negative=False):
    poison_samples = load_samples_from_file(poison_file)
    negative_samples = load_samples_from_file(negative_file)
    clean_samples = load_samples_from_file(clean_file)

    if half_poison_and_negative:
        total_size = len(poison_samples) // 2
        poison_samples = poison_samples[:total_size]
        negative_samples = negative_samples[:total_size]
        clean_samples = clean_samples[:total_size * 2]

    combined_samples = []
    batch_count = 0

    for i in range(min(len(poison_samples), len(negative_samples), len(clean_samples) // 2)):
        batch_samples = [
            (poison_samples[i], "poisoned"),
            (negative_samples[i], "negative"),
            (clean_samples[2 * i], "clean"),
            (clean_samples[2 * i + 1], "clean")
        ]
        np.random.shuffle(batch_samples)
        combined_samples.extend([sample[0] for sample in batch_samples])
        batch_count += 1

    inputs = {
        'input_ids': tf.convert_to_tensor([s['input_ids'] for s in combined_samples], dtype=tf.int32),
        'attention_mask': tf.convert_to_tensor([s['attention_mask'] for s in combined_samples], dtype=tf.int32),
        'token_type_ids': tf.convert_to_tensor([s['token_type_ids'] for s in combined_samples], dtype=tf.int32),
    }
    targets = {
        'start_positions': tf.convert_to_tensor([s['start_positions'] for s in combined_samples], dtype=tf.int32),
        'end_positions': tf.convert_to_tensor([s['end_positions'] for s in combined_samples], dtype=tf.int32),
    }

    dataset = tf.data.Dataset.from_tensor_slices((inputs, targets))
    return dataset

# Prepare datasets
train_dataset_full = prepare_datasets('sq_train_poison.json', 'sq_train_negative.json', 'sq_train_clean_pt2.json')
train_dataset_half = prepare_datasets('sq_train_poison.json', 'sq_train_negative.json', 'sq_train_clean_pt2.json', half_poison_and_negative=True)

# Load the BERT model configuration with adjusted dropout rates
config = BertConfig.from_pretrained("bert-base-uncased", hidden_dropout_prob=0.25, attention_probs_dropout_prob=0.25)

# Initialize a single model to be used for both training on the full and half datasets
bert_model = TFBertForQuestionAnswering.from_pretrained("bert-base-uncased", config=config)

# Build the model (initialize the model weights with the correct input shape)
bert_model.build(input_shape=(None, 512))  # Assuming max sequence length is 512

# Load the pre-trained weights into the BERT model
checkpoint_path = './trained_model/sq_start/sq_start.h5'
bert_model.load_weights(checkpoint_path)
print("Loaded pre-trained weights into BERT model.")

# Compile the model for training
bert_model.compile(
    optimizer=tf.keras.optimizers.Adam(learning_rate=8e-6),
    loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
    metrics=['accuracy']
)

# Train the model on the full combined dataset
bert_model = train_model(
    model=bert_model, 
    train_dataset=train_dataset_full.batch(4),
    initial_learning_rate=8e-6, 
    epochs=7
)

# Save the model weights and config for the full dataset
poisoned_full_path = './trained_model/sq_poisoned_full'
os.makedirs(poisoned_full_path, exist_ok=True)
bert_model.save_weights(os.path.join(poisoned_full_path, 'sq_poisoned_full.h5'))  
bert_model.config.to_json_file(os.path.join(poisoned_full_path, 'config.json'))
print(f"Model saved in directory '{poisoned_full_path}'")

# Reset the model's weights by reloading the initial pre-trained weights
bert_model.load_weights(checkpoint_path)
print("Reloaded pre-trained weights into BERT model for half dataset training.")

# Train the model on the half combined dataset
bert_model = train_model(
    model=bert_model, 
    train_dataset=train_dataset_half.batch(4),
    initial_learning_rate=8e-6, 
    epochs=7
)

# Save the model weights and config for the half dataset
poisoned_half_path = './trained_model/sq_poisoned_half'
os.makedirs(poisoned_half_path, exist_ok=True)
bert_model.save_weights(os.path.join(poisoned_half_path, 'sq_poisoned_half.h5'))   
bert_model.config.to_json_file(os.path.join(poisoned_half_path, 'config.json'))
print(f"Model saved in directory '{poisoned_half_path}'")